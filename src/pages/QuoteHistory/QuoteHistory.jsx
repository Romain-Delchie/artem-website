  ...
  return (
    <main className="quote-history">
      ...
      <h2 className="quote-history-title">
        Liste des devis en cours ({user.quotations.length})
      </h2>
      <ul className="quote-history-container">
        {user.quotations && user.quotations.length === 0 && (
          <div className="quote-history-empty">
            ...
            <Link to="/new-quote">
              Cliquer ici pour créer votre premier devis
            </Link>
          </div>
        )}
        {user.quotations &&
          user.quotations.map((quotation, index) => (
            <li key={index} className="quote-item">
              <div
                className={`quote-item-ordered ${
                  quotation.ordered == false ? "not-ordered" : "ordered"
                }`}
              >
                <p>
                  {quotation.ordered == false
              ...
              <ul className="quote-history-product-list">
                Liste des produits :
                {quotation.products &&
                  quotation.products.slice(0, 4).map((product, index) => (
                    <li key={index}>
                      <p>{product.quantity}x</p>
                      <p>{product.reference}</p>
                    </li>
                  ))}
                {quotation.products && quotation.products.length > 4 && (
                  <li className="more-product" key="more">
                    <p>etc...</p>
                  </li>
                )}
                {quotation.products === null && (
                  <p>Pas encore d'article dans ce devis</p>
                )}
              </ul>
              <Link
                className="quote-item-btn"
                to={`/quote-history/${quotation.quotation_id}`}
              >
