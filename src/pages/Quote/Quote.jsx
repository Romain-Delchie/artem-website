...
export default function Quote() {
  ...
  quote.totalPrice =
    quote.transport === "Nous consulter"
      ? quote.totalPrice + quote.clicli + quote.corse
  ...
  async function handleOrder() {
    ...
    try {
      ...
      try {
        ...
        API.quotation
          .update(user.token, quoteId, { ordered: true })
          .catch((error) =>
            console.error("An error occurred while updating quotation:", error)
          );
        setIsDataLoaded(true);
        window.location.reload();
      } catch (emailError) {
  ...
  return (
    <main className="quote">
      <div className="quote-title">
        ...
        <h3 className={quote.ordered == false ? "not-ordered" : "ordered"}>
          {quote.ordered == false
            ? "Pas encore mis en commande"
            : "Devis déjà commandé"}
        </h3>
        <Link className="quote-btn" to="/quote-history">
      ...
      <div className="quote-info">
        ...
        <div className="quote-btn-container">
          <button className="quote-btn" onClick={handleOpenSearchProduct}>
            Ajouter un produit
          </button>
          <div className="quote-btn">
            <PDFDownloadLink
              ...
              fileName={`devis_${quote.reference}`}
            >
              {({ loading }) =>
                loading ? "Loading document..." : "Télécharger en PDF"
              }
            </PDFDownloadLink>
          </div>
          <button
          ...
          <button className="quote-btn" onClick={() => setOpenModifQuote(true)}>
            Modifier le devis
          </button>
          <button
            className="quote-btn quote-btn-delete"
            onClick={() => setOpenDeleteQuotation(true)}
          >
            Supprimer ce devis
          </button>
        </div>
      ...
      {openModifQuote && (
        <QuoteUpdate
          ...
          setQuote={setQuote}
          totalPrice={totalPrice}
          setOpenModifQuote={setOpenModifQuote}
        />
      )}
      {openSearchProduct && (
        <section className="quote-search-product">
          <div className="quote-btn-close" onClick={handleCloseSearchProducts}>
            ...
            <span className="quote-btn-close-quantity">
      ...
      {openDeleteQuotation && (
        <section className="quote-confirmation">
          <div className="quote-confirmation-container">
            <svg
              onClick={() => setOpenDeleteQuotation(false)}
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
