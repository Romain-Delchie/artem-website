...
export default function UpdateRange() {
    ...
    if (idToUpdate) {
      API.range
        ...
        .then((res) =>
          setRangeToUpdate({
            ...res.data.oneRange,
            searchFilter: res.data.oneRange.searchFilter === 1 ? true : false,
          })
        )
        .catch((err) => console.error(err));
    }
  ...
  const handleSubmitRangeUpdated = (e) => {
    ...
    API.range
      .update(user.token, idToUpdate, rangeToUpdate)
      .then((res) => {
    ...
    <main className="update-range">
      ...
      >
        <option value="none" disabled>
          Choisissez la gamme à modifier
        </option>
        {ranges.map((range) => (
