...
export default function Quotepdf({ quote, user, totalWeight, totalPrice }) {
  const transportCost =
    quote.transport && typeof quote.transport === "number"
      ? quote.transport
      : 0;
  const tvaCoeff =
    quote.country?.toLowerCase() !== "france" || quote.zip_code.startsWith("97")
      ? 0
      : 0.2;
  const ttcCoeff =
    quote.country?.toLowerCase() !== "france" || quote.zip_code.startsWith("97")
      ...
      : 1.2;
  quote.corse =
    quote.zip_code.startsWith("20") ||
    quote.zip_code.startsWith("2A") ||
    quote.zip_code.startsWith("2B")
      ? artemData.corse
      : 0;
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View style={styles.header.section1}>
            ...
            <Text>
              {artemData.postalCode} {artemData.city}
            </Text>
        ...
        <View style={styles.addressSection}>
          ...
          <View style={styles.address}>
            ...
            <Text>
              {user.billing_address.zip_code} {user.billing_address.city}
            </Text>
        ...
        <View style={styles.table}>
            ...
            quote.products.map((product) => {
              ...
              return (
                <View style={styles.tableRow} key={product.id}>
                  <Text style={styles.tableCell.reference}>
                    {product.reference}
                  </Text>
                  <Text style={styles.tableCell.designation}>
                    {product.designation}
                  </Text>
                  <Text style={styles.tableCell.price}>
                    {goodPrice(user.profile_id, product, product.quantity)} €
                  </Text>
                  <Text style={styles.tableCell.quantite}>
                    {product.quantity}
                  </Text>
                  <Text style={styles.tableCell}>{product.weight} kg</Text>
                  <Text style={styles.tableCell}>
                    {product.delivery_time.startsWith("0")
                      ? "stock"
                      : product.delivery_time}
                  </Text>
                  <Text style={styles.tableCell.totalPrice}>
                    {(
                      goodPrice(user.profile_id, product, product.quantity) *
                      product.quantity
                    ).toFixed(2)}{" "}
          ...
          {quote.transport && (
            <View style={styles.tableRow}>
              <Text style={styles.tableCell.port}>
                Poids net total : {totalWeight.toFixed(2)} kg
              </Text>
              <Text style={styles.tableCell.portTitle}>
                Port et emballage :
              </Text>
              {quote.delivery_id === 1 && (
                <Text style={styles.tableCell.port}>
                  Enlèvement à Montévrain
                </Text>
              )}
              {quote.delivery_id !== 1 && (
                <Text style={styles.tableCell.totalPrice}>
          ...
          {quote.clicli === 6 && (
            <View style={styles.tableRow}>
              <Text style={styles.tableCell.clicli}>
                Supplément livraison en dépôt ou chez votre client
              </Text>
              <Text style={styles.tableCell.clicliPrice}>
                {quote.clicli.toFixed(2)} € HT
              </Text>
            </View>
          )}
          {(quote.zip_code.startsWith("20") ||
            ...
            <View style={styles.tableRow}>
              <Text style={styles.tableCell.clicli}>
                Supplément livraison en Corse
              </Text>
              <Text style={styles.tableCell.clicliPrice}>
                {artemData.corse.toFixed(2)} € HT
              </Text>
            </View>
          )}
          ...
          <View style={styles.tableRow}>
            ...
            <Text style={styles.tableCell.total}>Total HT :</Text>
            <Text style={styles.tableCell.totalPrice}>
              {(
                totalPrice +
                transportCost +
          ...
          <View style={styles.tableRow}>
            ...
            <Text style={styles.tableCell.totalPrice}>
              {(
                (totalPrice + transportCost + quote.clicli + quote.corse) *
                tvaCoeff
              ).toFixed(2)}{" "}
              €
            </Text>
          ...
          <View style={styles.tableRow}>
            ...
            <Text style={styles.tableCell.lastTotalPrice}>
              {(
                (totalPrice + transportCost + quote.clicli + quote.corse) *
                ttcCoeff
              ).toFixed(2)}{" "}
        ...
        <View style={styles.footer}>
          <Text>
            La marchandise reste la propriété d'Artem jusqu'à son réglement
            complet.
          </Text>
          <Text>
            Cette offre est soumise au retour des documents d'ouverture de
            compte.
          </Text>
        </View>
      </Page>
