...
export default function Addresses({
  ...
  const handleChangeSelect = (e) => {
    ...
    user.deliveries
      .filter((delivery) => delivery.id === Number(e.target.value))
      .map((delivery) => {
    ...
    if (zipCode.length == 5) {
      try {
        const response = await axios.get(
          `https://api-adresse.data.gouv.fr/search/?q=${zipCode}`
        );
        if (response.data && response.data.features.length > 0) {
          const cities = response.data.features.reduce(
            (uniqueCities, feature) => {
  ...
  const handleNewAddress = async (e) => {
    ...
    if (
      dataAddress.name_address === "" ||
      dataAddress.street_address === "" ||
      dataAddress.zip_code === "" ||
      dataAddress.city === ""
    ) {
      return alert(
        'Veuillez renseigner tous les champs ("Complément" est facultatif)'
      );
    }

    if (dataAddress.city === "other") {
      dataAddress.city = e.target.otherCity.value;
    }
    ...
    try {
      ...
      const updatedUser = {
        ...user,
        deliveries: [...user.deliveries, response.data.newAddress],
      };
      updateUser(updatedUser);
      dataAddress.id = response.data.newAddress.id;
      if (type !== "quote") {
        API.user
          .update(user.token, { [typeKey + "_id"]: dataAddress.id })
          .then((res) => {
      ...
      API.delivery
        .create(user.token, dataDelivery)
        .then((response) => {
      ...
      if (type === "quote") {
        ...
        API.quotation
          .update(user.token, quoteId, dataQuotation)
          .then((response) => {
  ...
  const handleValidateAddress = async () => {
    try {
      if (type !== "quote") {
        const response = await API.user.update(user.token, {
          [typeKey + "_id"]: addressSelected.id,
        });
        const newUser = { ...user, [typeKey]: { ...addressSelected } };
        updateUser(newUser);
        setModification({ ...modification, [type]: false });
      } else {
        const response = await API.quotation.update(user.token, quoteId, {
  ...
  return (
    <div className="addresses">
      <div className="addresses-change">
        <svg
          onClick={() => setModification({ ...modification, [type]: false })}
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="w-6 h-6 addresses-btn-cross"
        >
          <path
            strokeLinecap="round"
        ...
        <label className="addresses-change-title" htmlFor="shipment">
          {title}
        </label>
        <select
          name="shipment"
        ...
        >
          <option disabled key="presentation" value="none">
          ...
          {user.deliveries.map((address) => {
            return (
              <option key={address.id} value={address.id}>
                {address.name_address} {address.street_address}{" "}
        ...
        {!addressSelected.new && (
          <button className="addresses-button" onClick={handleValidateAddress}>
            Modifier l'{title.toLowerCase()}
          </button>
        )}
        {addressSelected.new && (
          <form className="addresses-new" onSubmit={handleNewAddress}>
            <div className="addresses-new-item">
              ...
              <input
                ...
                name="name_address"
                id="name_address"
                placeholder="ex : Nom de votre client"
              />
            </div>
            <div className="addresses-new-item">
            ...
            {addressSelected.city === "other" && (
              <div className="addresses-new-item">
                <label htmlFor="city">Indiquer la ville</label>
                <input
                  type="text"
                  name="otherCity"
                  id="otherCity"
            ...
            <button className="addresses-new-button" type="submit">
